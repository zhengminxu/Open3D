...
def test_voxelize():
    ...
    in_points = torch.tensor(np.load("res.npy"), dtype=torch.float32, device=torch.device('cuda:0'))
...
def test_nms():
    ...
    in_boxes = torch.tensor([
        [15.0811, -7.9803, 15.6721, -6.8714,  0.5152],
        ...
        [15.0931, -7.9552, 15.6675, -7.0056,  0.5979]], dtype=torch.float32, device=torch.device('cuda:0'))
    in_scores = torch.tensor([0.1616, 0.1556, 0.1520, 0.1501, 0.1336, 0.1298], dtype=torch.float32, device=torch.device('cuda:0'))
    in_thrs = 0.01
